...
| social.json | 社交主题 | #1877F2 | Facebook风格，简洁现代，友好美观 | 社交应用场景，喜欢Facebook风格设计 |
...
5. 修改主题前建议备份原始文件
6. **更新sources.json时必须更新update_time字段**：
   - `update_time`字段应填写当前时间戳（Unix timestamp，单位为秒）
   - 获取时间戳的方法：
     - macOS/Linux：`date +%s`
     - Windows：`powershell -Command "[int][double]::Parse((Get-Date -UFormat %s))"`
     - 在线工具：https://www.unixtimestamp.com/
   - 示例：`"update_time": 1766995701`
   - **重要**：每次修改sources.json或sources.zh.json时，必须同步更新update_time字段为当前时间戳，否则可能导致主题更新无法被应用正确识别
...
**更新说明**：
- v1.10：新增social主题，采用Facebook设计风格，使用标志性的蓝色主色调和简洁的社交媒体设计；添加update_time字段使用说明，提醒开发者更新主题列表时必须同步更新时间戳
